...
import payloadData from '../data/grant-payment-wmp-payload_01.json'
import { faker } from '@faker-js/faker'
import { isEnvTrue } from '../services/env-flags.js'
import { replaceDatesWithFuture } from '../helper/date_helper.js'

const wmpDisabled = isEnvTrue('WMP_ENABLED')
const wmpDescribe = wmpDisabled ? describe : describe.skip

if (!wmpDisabled) {
  console.log(
    'SKIPPING: WMP tests - WMP_ENABLED environment variable is not set to true'
  )
}

console.log('wmpDescribe', wmpDescribe)
wmpDescribe(
  'Grants Payment Service - Store and Process Payments for WMP',
  () => {
    let testClaimId
    let setupPayload
